describe('User Profile and Account Deletion', () => {
    ...
    beforeEach(() => {
      ...
      cy.get('#username').type('adminuser'); // Replace with the actual username
      cy.get('#password').type('adminpassword123'); // Replace with the correct password
      cy.get('button[type="submit"]').click();
    });
  
    it('should display the user profile and then delete the account', () => {
      // Step 1: Navigate to the user dashboard
      cy.url().should('include', 'http://localhost:3000/admin/dashboard'); // Ensure we are on the dashboard page
  
      // Step 2: Open the profile menu
      // Find the Avatar containing the AccountCircleIcon and click it
      cy.get('button').find('svg[data-testid="AccountCircleIcon"]').click({ multiple: true });
      ...
      cy.url().should('include', 'http://localhost:3000/admin/profile'); // Ensure we are on the profile page
      cy.contains('User Information').should('be.visible');
      cy.get('body').should('contain', 'Username: adminuser'); // Adjust based on the actual username
      ...
      cy.get('input[type="password"]').type('adminpassword123'); // Replace with the correct password
